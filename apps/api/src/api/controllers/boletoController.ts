...
export async function gerarSegundaVia(req: Request, res: Response) {
  try {
    const { fatura_id, id } = req.params;
    const targetId = fatura_id || id;
    ...
    if (userIds && userIds.length > 0) {
      ...
      for (const idCliente of userIds) {
        ...
        if (faturas.some((f: any) => String(f.id) === String(targetId))) {
          pertence = true;
          break;
        }
      }
    ...
    const base64 = await ixcService.imprimirBoleto(Number(targetId));
